...
int main(void)
{
	...
	numRounds = ceil((double)(contestantNum + 16) / 2);
    ...
    for (; sims > 0; --sims) //you think this is bad? I could have used while(sims-->0). be thankful.
    {
        ...
        while (winner == NULL)
        {
            ...
            int killnum = round((double)(alivenum) * 1/3); //lose 1 life for 1/3- NR.
			...
			int ranking = 0;
            for (int i = 0; i < contestantNum; ++i) //update survival rates to new data, update points
            {
                if (contestants[i].alive)
				{
